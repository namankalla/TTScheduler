/**
 * Qwen 2.3 AI Service via OpenRouter
 * Parses timetable text into structured JSON
 */

const QWEN_API_KEY = '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';
const QWEN_API_URL = 'https://openrouter.ai/api/v1/chat/completions';

export const parseTimetableText = async (ocrText) => {
  const models = [
  ...
  for (let i = 0; i < models.length; i++) {
    try {
      console.log(`Starting AI parsing with model: ${models[i]}...`);

      const prompt = buildTimetablePrompt(ocrText);
      ...
      const response = await fetch(QWEN_API_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are an expert at parsing academic timetables and schedules. Always respond with valid JSON only, no additional text.'
            },
            {
              role: 'user',
              content: prompt
      ...
      console.log(`AI parsing completed with model: ${models[i]}`);
      ...
      if (parsedData.courses && parsedData.courses.length > 0) {
        ...
        console.warn(`Model ${models[i]} returned empty results, trying next model...`);
        continue;
      }

    } catch (error) {
      console.error(`AI parsing failed with model ${models[i]}:`, error);
      
      // If this is the last model, throw the error
      if (i === models.length - 1) {
        throw new Error(`AI parsing failed with all models: ${error.message}`);
      }
 ...
 */
const buildTimetablePrompt = (ocrText) => {
  return `
...
1. TIME SLOT MAPPING (EXACT TIMES FROM TIMETABLE):
...
2. TIME CONVERSION RULES:
   - Times 12:00-12:59 = 12:00-12:59 (noon)
   - Times 01:00-11:59 = 13:00-23:59 (afternoon/evening)
   - Times 09:00-11:59 = 09:00-11:59 (morning)
   - NEVER use times like 01:00, 02:00, etc. for morning classes
   - ALL classes are during normal university hours (9 AM to 5 PM)

3. COURSE PATTERN RECOGNITION:
...
5. DAY MAPPING (EXACT ORDER):
   - Monday, Tuesday, Wednesday, Thursday, Friday, Saturday
   - Match courses to their respective days in the grid columns

6. LOCATION EXTRACTION:
   - Room numbers: "203-A", "172(C8)", "202", etc.
   - Lab codes: "172(C8)", "203-A", etc.

7. COURSE NAME MAPPING:
...
8. INSTRUCTOR EXTRACTION:
   - From parentheses: (ASD), (SSV), (MAI), (VAT)
   - From complex codes: TDPIT4, etc.
...
11. COMPLEX PATTERNS:
    - Handle split entries: "7A12:CS:B1:TDPIT4:172(C8)" and "7A12:CS:B2:TDPIT4:172(C8)"
    - Handle multiple locations: "203-A", "172(C8)"
    - Handle time ranges: "09:30-10:25"
...
const parseAIResponse = (aiResponse) => {
  try {
    ...
    console.log('AI parsing failed, attempting basic fallback parsing...');
 ...
 */
export const getConfidenceScore = (parsedData) => {
  if (!parsedData.courses || parsedData.courses.length === 0) {
    return 0;
  }
